...
describe("Modified", () => {
  ...
  //   const initialExternalRatio =
  //     await liquiditypool.getChainlinkDataFeedLatestAnswer();
  //   console.log(initialExternalRatio);
  ...
  //   assert.equal(initialExternalRatio, 0);
  //   assert.equal(newExternalRatio, 100);
  // });

  // it("should gather data points every 2 swaps", async () => {
  //   const [owner] = await ethers.getSigners();
  ...
  //   assert.equal(initialAverageRatio, 0);
  //   assert.equal(lastFetchedRatio, 0);
  ...
  //   }
  //   console.log("\n\n\n\n");
  //   //CHECK AVERAGE RATIO AFTER TWO TRANSACTIONS
  //   const averageRatio = await liquiditypool.average_external_ratio();
  ...
  //   }
  //   const newestFetchedRatio = await liquiditypool.lastFetchedExternalRatio();
  //   const newestAverageRatio = await liquiditypool.average_external_ratio();
  //   const newDataPoints = await liquiditypool.getDataPoints();
  //   console.log(newDataPoints);
  //   assert.equal(newestAverageRatio, parseEther("1.25"));
  //   assert.equal(newestFetchedRatio, parseEther("1.3"));
  ...
  //     parseEther("1.23"),
  //   ];
  //   // run 10 swaps, after every 2 swaps, change the external ratio
  //   for (let i = 0; i < 10; i++) {
  //     if ((i + 1) % 2 == 0 && i != 0) {
  //       console.log("Iteration: ", i);
  //       const index = Math.floor(i / 2); // Use integer division to get the index
  //       console.log(index);
  ...
  //   //set an external ratio
  //   await liquiditypool.setExternalRatio(parseEther("1.2"));
  //   //check the dataPoints (should be empty);
  //   const oldDataPoints = await liquiditypool.getDataPoints();
  //   assert.equal(oldDataPoints.length, 0);
  //   await ethers.provider.send("evm_increaseTime", [61]);
  //   //run a swap (time diff should trigger the fetch of new external ratio);
  ...
  //   console.log("external ratio: ", externalRatio);
  //   assert.equal(internalFee, BigInt(3 * 1e15));
  ...
  it("should use the modified fee when reserve ratios are not close", async () => {
    ...
    //check the internal fee based on this
    const internalFee = await liquiditypool.baseFee();
    assert.equal(internalFee, BigInt(5e17));
    assert.equal(externalRatio, parseEther("2.0"));
    console.log("intenal ratio: ", internalRatio);
    console.log("external ratio: ", externalRatio);
  });
